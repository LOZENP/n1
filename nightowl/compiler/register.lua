...
return function(Compiler)
    ...
    function Compiler:registerList(scope, ids)
        local l = {}
        for i, id in ipairs(ids) do l[i] = self:register(scope, id) end
        return l
    end
    ...
    function Compiler:setRegisters(scope, ids, vals)
        local lhs = {}
        for i, id in ipairs(ids) do lhs[i] = self:registerAssignment(scope, id) end
    ...
    function Compiler:copyRegisters(scope, to, from)
        ...
        for i, id in ipairs(to) do
            local fromId = from[i]
            if fromId ~= id then
                table.insert(lhs,  self:registerAssignment(scope, id))
    ...
    function Compiler:setPos(scope, val)
        if not val then
            ...
            return Ast.AssignmentStatement(
                {Ast.AssignmentVariable(self.containerFuncScope, self.posVar)},
                {Ast.IndexExpression(self:env(scope), Ast.StringExpression("__owl_nil_"..tostring(math.random(1,9999))))}
            )
        end
        scope:addReferenceToHigherScope(self.containerFuncScope, self.posVar)
        return Ast.AssignmentStatement(
