...
return function(self, expression, funcDepth, numReturns)
    ...
    if expression.rhs.isConstant then
        ...
        self:addStatement(self:setRegister(scope, resReg, Ast.OrExpression(self:register(scope, lReg), self:register(scope, rReg))), {resReg}, {lReg, rReg}, false)
    ...
    self:addStatement(self:setRegister(scope, self.POS_REGISTER,
        Ast.OrExpression(Ast.AndExpression(self:register(scope, lReg), Ast.NumberExpression(b2.id)), Ast.NumberExpression(b1.id))
